              ...
              {node.recommendedResource && (resourcePricingFilter === 'all' || resourcePricingFilter === 'free') && (
                <div className="relative rounded-2xl border border-violet-500/40 bg-violet-500/[0.05] p-5 shadow-sm">
                  ...
                  <div className="mt-4 pt-4 border-t border-violet-500/20 flex items-center justify-between gap-3">
                    ...
                    <a
                      href={node.recommendedResource.url || `https://www.google.com/search?q=${encodeURIComponent(node.recommendedResource.title || node.title)}`}
                      target="_blank"
                      rel="noreferrer"
                      ...
                      <div key={video.id} className="group flex gap-4 items-start py-3.5 first:pt-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center justify-between gap-2 mt-2">
                            ...
                            <a
                              href={video.url || `https://www.youtube.com/results?search_query=${encodeURIComponent(video.title || node.title)}`}
                              target="_blank"
                              rel="noreferrer"
                ...
                <section className="space-y-2">
                  ...
                  <div className="divide-y divide-slate-200 dark:divide-white/[0.06]">
                    {node.articles.map((art, idx) => (
                      <a
                        key={idx}
                        href={art.url || `https://www.google.com/search?q=${encodeURIComponent(art.title || node.title)}`}
                        target="_blank"
                        rel="noreferrer"
                        className="flex items-center justify-between gap-4 py-2.5 group hover:text-violet-600 dark:hover:text-violet-400 transition"
                      >
                        <span className="text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300 group-hover:text-violet-600 dark:group-hover:text-violet-400 truncate">{art.title}</span>
                        <span className="mono-label text-slate-400 dark:text-slate-500 shrink-0 text-[10px] flex items-center gap-1">
                          {art.duration} <ExternalLink className="w-3 h-3 text-slate-400 group-hover:text-violet-500" />
                        </span>
                      </a>
                    ))}
                  </div>
