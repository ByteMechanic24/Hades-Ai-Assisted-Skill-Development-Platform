...
export function AuthProvider({ children }) {
  ...
  const login = useCallback(async ({ email, password }) => {
    ...
    try {
      ...
      const msg = err.body?.error?.message || err.body?.message || (typeof err.body?.error === 'string' ? err.body.error : null) || 'Sign in failed. Check your credentials.';
      setAuthError(msg);
      throw err;
    } finally {
  ...
  const logout = useCallback(() => {
    ...
    setAuthError(null);
    try {
      localStorage.removeItem('hades_auth_user');
      localStorage.removeItem('hades_has_generated_roadmap');
      localStorage.removeItem('hades_active_learning_path');
      localStorage.removeItem('hades_active_roadmap');
      localStorage.removeItem('hades_all_roadmaps');
      localStorage.removeItem('hades_recent_events');
      sessionStorage.clear();
    } catch (e) {
      console.warn('[HADES] Storage purge on logout warning:', e);
    }
  }, []);
