...
export function OnboardingPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    customGoal: initialRole ? `Master ${initialRole} in 8 Weeks` : '',
    timeframeWeeks: 8,
  ...
  const stepQuestions = {
    1: {
      ...
      title: 'What specialization or topic are you targeting?',
      desc: 'Select a domain or type your exact target learning topic/career path.',
    },
    2: {
      eyebrow: 'Step 2 of 5 · Technical Stack',
      title: 'Which technologies and domains do you want to master?',
      desc: 'Select existing topics or type your own custom topics and tools below.',
    },
    3: {
  ...
  const roleOptions = [
    { id: 'Machine Learning', title: 'Machine Learning', category: 'AI & Predictive Modeling' },
    { id: 'Cybersecurity', title: 'Cybersecurity', category: 'Network Defense & Threat Hunting' },
    { id: 'Cloud Computing', title: 'Cloud Computing', category: 'AWS, Cloud Infra & DevOps' },
    ...
    { id: 'Full-Stack LLM Developer', title: 'Full-Stack LLM Developer', category: 'FastAPI, React & Streaming' },
  ];

  const baselineOptions = [
    { id: 'Beginner', title: 'Beginner', desc: 'Starting from fundamental principles.' },
    { id: 'Intermediate', title: 'Intermediate', desc: 'Working experience with core concepts and tools.' },
    { id: 'Advanced', title: 'Advanced', desc: 'Proficient practitioner aiming for architecture mastery.' }
  ];
  ...
  const handleStepForward = () => {
    if (currentStep === 1 && (!formData.targetRole || !formData.targetRole.trim())) {
      setGenerationError('Please select a quick suggestion or type your target topic before continuing.');
      return;
    }
    setGenerationError(null);
    if (currentStep < totalSteps) {
      setCurrentStep((p) => p + 1);
    } else if (currentStep === totalSteps && !isGenerating) {
  ...
  const handleAddCustomTopic = () => {
    ...
    if (!trimmed) return;
    setFormData((prev) => {
      const interests = prev.interests.includes(trimmed) ? prev.interests : [...prev.interests, trimmed];
      const customTopics = (prev.customTopics || []).includes(trimmed) ? prev.customTopics : [...(prev.customTopics || []), trimmed];
      return { ...prev, interests, customTopics };
    });
    setCustomTopicInput('');
  };
  ...
  const handleRemoveCustomTopic = (topic) => {
    setFormData((prev) => ({
      ...
      customTopics: (prev.customTopics || []).filter((ct) => ct !== topic),
    }));
  };
  ...
  const handleFinishOnboarding = async () => {
    ...
    if (import.meta.env.DEV) {
      console.log('[HADES][ONBOARDING]', {
        'goal/topic': formData.targetRole,
        interests: formData.interests,
        customTopics: formData.customTopics || [],
        experience: formData.experienceLevel,
        preferences: formData.learningFormats,
        weeklyHours: formData.weeklyHours,
      });
    }

    const stageTimers = [
      setTimeout(() => setGenerationStage(2), 1200),
      setTimeout(() => setGenerationStage(3), 2400),
    ];
    ...
    try {
      ...
      setGenerationError(err.body?.message || err.message || 'Something went wrong while generating your learning path. Please try again.');
                ...
                <div className="flex-1 min-h-0 my-3 rounded-xl border border-slate-200/80 dark:border-white/[0.08] bg-slate-50/50 dark:bg-white/[0.02] p-3.5 sm:p-4 overflow-hidden flex flex-col">
                  ...
                  >
                    <AnimatePresence mode="wait">
                      ...
                      >
                        {/* STEP 1: Role Specialization / Learning Topic */}
                        {currentStep === 1 && (
                          <div className="space-y-4">
                            <div>
                              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
                                Select Quick Suggestion or Type Custom Target Topic:
                              </label>
                              <div className="flex flex-wrap gap-2 mb-3">
                                {roleOptions.map((role) => {
                                  const selected = Boolean(formData.targetRole && formData.targetRole.toLowerCase() === role.id.toLowerCase());
                                  return (
                                    <button
                                      key={role.id}
                                      type="button"
                                      onClick={() => {
                                        const newRole = selected ? '' : role.id;
                                        setFormData((prev) => ({
                                          ...prev,
                                          targetRole: newRole,
                                          customGoal: newRole ? `Master ${newRole} in ${prev.timeframeWeeks || 8} Weeks` : '',
                                        }));
                                      }}
                                      className={`inline-flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs sm:text-[13px] font-semibold border transition-all ${
                                        selected
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  setFormData((prev) => ({
                                    ...prev,
                                    targetRole: val,
                                    customGoal: val ? `Master ${val} in ${prev.timeframeWeeks || 8} Weeks` : '',
                                  }));
                                }}
                                className={inputCls}
                                placeholder="Type custom topic e.g. Cybersecurity, Agentic AI, Cloud DevOps, Full-Stack..."
                              />
                            </div>

                            {/* User-Controlled Timeframe (Weeks) & Specific Goal */}
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              <div>
                                ...
                                <div className="relative">
                                  <input
                                    ...
                                    onChange={(e) => {
                                      const weeks = parseInt(e.target.value, 10) || 0;
                                      setFormData((prev) => ({
                                        ...prev,
                                        timeframeWeeks: weeks,
                                        customGoal: prev.targetRole ? `Master ${prev.targetRole} in ${weeks} Weeks` : prev.customGoal,
                                      }));
                                    }}
                                    className={inputCls}
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  className={inputCls}
                                  placeholder="e.g. Master Cybersecurity & Ethical Hacking"
                                />
                              </div>
                            </div>
                          </div>
                        ...
                        {/* STEP 2: Technical Stack & Custom Topics */}
                        {currentStep === 2 && (
                          <div className="space-y-3">
                            ...
                            <div className="pt-3 border-t border-slate-200/80 dark:border-white/[0.08]">
                              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
                                Add your own topic / custom interest:
                              </label>
                              <div className="flex gap-2">
                                <input
                                  ...
                                  className={inputCls}
                                  placeholder="e.g. Docker, Kubernetes, System Design, LangChain, SIEM..."
                                />
                                <button
                                  type="button"
                                  onClick={handleAddCustomTopic}
                                  className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs transition-colors shrink-0 flex items-center gap-1 shadow-sm"
                                >
                                  <Plus className="w-3.5 h-3.5" /> Add
                                </button>
                              </div>

                              {formData.customTopics && formData.customTopics.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mt-2.5">
                                  {formData.customTopics.map((ct) => (
                                    <span
                                      key={ct}
                                    ...
                                    >
                                      ...
                                      <button
                                        type="button"
                                        onClick={() => handleRemoveCustomTopic(ct)}
                                        className="hover:text-rose-500 transition-colors ml-0.5 text-sm leading-none"
                                      >
                                        ×
                                      </button>
                                    </span>
