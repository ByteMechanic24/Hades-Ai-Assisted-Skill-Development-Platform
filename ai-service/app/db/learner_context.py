...
def get_learner_context(
    ...
    try:
        ...
        logger.info("Learner '%s' not found in database; using dynamic fallback context.", cleaned_external_id)
        return LearnerProfile(
            learner_id=cleaned_external_id,
            target_goal="",
            career_aspirations=["Full-Stack Engineer", "AI Systems Engineer"],
            current_skills=[SkillItem(skill_name="Programming Basics", level="intermediate", years_of_experience=1.0)],
            interests=["Generative AI", "Vector Databases", "Agentic Systems"],
        ...
        logger.warn("Database query encountered error in get_learner_context (%s); using dynamic fallback context.", sanitized_msg)
        return LearnerProfile(
            learner_id=cleaned_external_id,
            target_goal="",
            career_aspirations=["Full-Stack Engineer", "AI Systems Engineer"],
