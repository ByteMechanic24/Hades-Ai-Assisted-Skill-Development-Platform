...
class AIOrchestrator:
    ...
    def _hydrate_initial_state(
        ...
        saved_chunks = (
            get_saved_roadmap_chunks(learner_id)
            if request.event != "INITIAL_SESSION"
            else []
        )
    ...
    def _execute_action(
        ...
        if action == "ERROR_RECOVERY":
            logger.info("[ORCHESTRATOR EXECUTE] Performing resilient error recovery.")
            if not state.skill_analysis:
                try:
                    state.skill_analysis = analyze_learner_skills(learner_id=state.learner_id, target_goal=state.target_goal)
                except Exception as ex:
                    logger.warning("[ORCHESTRATOR] Fallback skill analysis error: %s", ex)
            if not state.generated_chunks:
                try:
                    ...
                    if state.available_topics:
                        state.active_topic = state.available_topics[0]
                        state.current_topic_index = 0
                except Exception as ex:
                    logger.warning("[ORCHESTRATOR] Fallback chunk generation error: %s", ex)
            state.status = "READY"
            return False
